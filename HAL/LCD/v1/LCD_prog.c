...
void LCD_vInit (void)
{
	...
	LCD_u8WriteCommand (LCD_u8_INIT_FUNCTION_SET);
	delay_milliseconds (2);
	LCD_u8WriteCommand (LCD_u8_INIT_DISPLAY_CONTROL);
	delay_milliseconds (2);
	LCD_u8WriteCommand (LCD_u8_CLEAR);
	delay_milliseconds (2);
}
...
u8 LCD_u8WriteCommand (u8 Copy_u8Command)
{
	...
	Local_u8Error = u8Write (Copy_u8Command);
	/*Function return*/
...
u8 LCD_u8WriteData (u8 Copy_u8Data)
{
	...
	Local_u8Error = DIO_u8SetPinValue(LCD_u8_RS_PIN,DIO_u8_PIN_HIGH);
	Local_u8Error = u8Write(Copy_u8Data);
